...
class SearchResult:
    ...
    def __init__(self, path, modified_time):
        self.path = path
        self.modified = time.strftime("%D %H:%M:%S", time.gmtime(modified_time))
...
class MainPanel(wx.Panel):
    ...
    def create_ui(self):
        ...
        # Add search bar
        self.search_ctrl = wx.SearchCtrl(self, style=wx.TE_PROCESS_ENTER, size=(-1, 25))
        ...
        if result:
            ...
            try:
                ...
                with wx.MessageDialog(
                    None, message=message, caption="Error", style=wx.ICON_ERROR
                ) as dlg:
                    dlg.ShowModal()
    ...
    def search(self, search_term, file_type):
        ...
        if folder:
            ...
            SearchSubdirectoriesThread(
                folder, search_term, file_type, self.case_sensitive.GetValue()
            )
...
class Search(wx.Frame):
    ...
    def __init__(self):
        super().__init__(None, title="Search Utility", size=(600, 600))
